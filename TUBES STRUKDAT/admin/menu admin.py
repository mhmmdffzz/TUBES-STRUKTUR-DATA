...
from database.data_store import users_list, artists_list #disini kita bakal masukin/import si asrtist list dan user list agar si user bisa akses data lagu dan artis
from database.models import User

# --- FUNGSI BANTUAN (Lokal di file ini) ---
def clear_screen():
    os.system('cls' if os.name == 'nt' else 'clear') #bersihin terminal kalau kita milih salah satu menu/opsi
...
def user_add_song_logic(current_user):
    ...
    found_artist = None  #kenapa none? biar bisa dicek di bawah apakah ada atau enggak lagunya
    for artist in artists_list:
        if artist.name.lower() == artist_name.lower():
            found_artist = artist
            break  # Ketemu artis, keluar dari loop
